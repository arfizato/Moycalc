<script lang="ts">
	...
	onMount(() => {
		...
		try {
			const finishedTutorial = localStorage.getItem('finished-create-tutorial');
						...
						steps: [
							{
								...
								popover: {
									title: 'Regime Mixte',
									description:
										'Standard subjects with a mid-semester test (coefficient 0.3) and final exam (coefficient 0.7). This is the most common type in Tunisia.'
								}
							},
							{
								element: '#regmixCoef',
								popover: {
									title: 'Configure Test Coefficients',
									description:
										'Here you can configure how important each test is. DS (Devoir Surveillé) is the mid-semester test, Exam is the final exam. By default they are 0.3 and 0.7, but you can adjust them. The values should add up to 1.0 (or 100%).'
								}
							},
							{
...

							{
								...
								popover: {
									...
									description:
										'Alternative subjects graded on presence, homework, presentations, and 2 small tests. Used for group projects and optional courses.'
								}
							},
							{
								element: '#ccCoef',
							...
							},
							{
								element: '#addCcBtn',
								popover: {
									title: 'Add CC Subjects',
									description:
										'Click here to add subjects to Contrôle Continue. Fill in the subject name and coefficient.'
								}
							},
							{
								element: '#createBtn',
	...
	async function createTemplate() {
		let isInputValid: Boolean =
			(regmix['subjects'].length > 0 || cc['subjects'].length > 0) &&
			isSubjectValid(regmix['subjects'], 'RM') &&
			isSubjectValid(cc['subjects'], 'CC') &&
			isCoefValid(regmix['coef'], regmixCoefNames) &&
			isCoefValid(cc['coef'], ccCoefNames);
		if (!isInputValid)
			return Swal.fire({
				title: 'Error!',
				text: 'Invalid Input',
				...
				customClass: {
					confirmButton:
						'bg-transparent text-white py-2 px-4 hover:bg-white hover:text-black transition-all duration-200 border-2 border-white rounded-md'
				}
			});
		...
		setTimeout(async () => {
			...
			const { value: name } = await Swal.fire({
				...
				customClass: {
					confirmButton:
						'bg-transparent text-white py-2 px-4 hover:bg-white hover:text-black transition-all duration-200 border-2 border-white rounded-md  mx-2 ',
					cancelButton:
						'bg-zinc-800 text-white py-2 px-4 hover:bg-white hover:text-black transition-all duration-200 border-2 border-white rounded-md mx-2',
...
<section
	id="createMain"
	class="w-full h-screen min-h-fit grid justify-items-center bg-black"
	on:mousemove={(e) => handleMouseMove(e)}
>
	<div id="createFormContainer" class="h-fit min-h-3/4 w-3/4 bg-zinc-800 rounded mt-24">
		<form
			id="form"
			class="rounded flex flex-col justify-evenly"
			on:keypress={(event) => handleKeyDown(event)}
			name="create"
			data-netlify="true"
		>
			<h1
				class="text-3xl py-4 px-6 text-white uppercase w-full text-center glookFont merriweatherSansFont"
			>
				Create Your own template
			</h1>
			<div id="regmix" class="my-4 w-full">
				<h2
					class="text-xl py-4 text-white uppercase w-full text-center glookFont merriweatherSansFont"
				>
					Regime Mixte
				</h2>
				<h2
					class="text-lg py-4 text-white uppercase w-full text-center glookFont merriweatherSansFont"
				>
					Coef
				</h2>
				<div
					id="regmixCoef"
					class="mx-4 my-2 grid items-center justify-items-center grid-cols-3 text-base text-white gap-2"
				>
					{#each regmix['coef'] as coef, i}
						<div class="flex flex-col">
							...
							<input
								type="number"
								bind:value={coef}
								id={regmixCoefNames[i]}
								class="bg-inherit p-1 border-b-2 border-b-zinc-800 w-full"
							/>
						</div>
					{/each}
				</div>
				...
				>
					...
					{#each regmix['subjects'] as subject, i}
						<p
							class={'mt-4 w-3/4 grid grid-col text-center regmix' + i}
							transition:slide={{ delay: 0, duration: 150, easing: quintOut }}
						>
							<button
								on:click|preventDefault={() => delRow('regmix' + i, i)}
						...
						<input
							...
							placeholder="subject"
							transition:slide={{ delay: 0, duration: 150, easing: quintOut }}
							class={'w-full md:w-3/4 mt-4 bg-inherit py-1 border-b-2 border-b-zinc-800  regmix' +
								i}
						/>
						<input
							type="number"
							bind:value={subject['coef']}
							id={'RMcoef' + i}
							placeholder="coef"
							transition:slide={{ delay: 0, duration: 150, easing: quintOut }}
							class={'w-full md:w-3/4 mt-4 bg-inherit py-1 border-b-2 border-b-zinc-800  regmix' +
				...
				<div class="w-full flex justify-center">
					<button
						id="addRegmixBtn"
						on:click|preventDefault={() => addRow(true)}
			...
			<div id="cc" class="my-4 w-full">
				...
				>
					...
					{#each cc['subjects'] as subject, i}
						<p
							class={'mt-4 w-3/4 grid grid-col text-center cc' + i}
							transition:slide={{ delay: 0, duration: 150, easing: quintOut }}
						>
							<button
								on:click|preventDefault={() => delRow('cc' + i, i)}
				...
				<div class="w-full flex justify-center">
					<button
						id="addCcBtn"
						on:click|preventDefault={() => addRow(false)}
						class="addRowBtn bg-transparent border-2 border-zinc-200 text-zinc-200 disabled:bg-zinc-800 text-base mt-4 mb-8 py-2 px-8
                            rounded-md hover:bg-zinc-200 hover:text-black relative transition-all duration-200 z-10
                            focus:bg-zinc-200 focus:text-black"
					>
			...
			<div class="w-full flex items-center justify-center gap-4 my-6">
				<a
					href="/"
					class="py-2 px-1 w-1/6 min-w-max text-center text-base bg-transparent border-2 border-zinc-600 text-zinc-600
				...
				<button
					id="createBtn"
					on:click|preventDefault={createTemplate}
